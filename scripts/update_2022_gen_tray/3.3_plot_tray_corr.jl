...
loadpath = datadir("results", "2022_tray_infl","tray_infl", "corr")
...
df               = innerjoin(df, components(optcorr2022), on=:measure)
df[!,:w_tray]    = df.tray_infl .* df.weights

temp = DataFrame(measure = optcorr2022.name, tray_infl=[sum(df.w_tray)])
...
for i in 1:8
    ...
    display(p)
    savefig("C:\\Users\\DJGM\\Desktop\\PLOTS\\2022\\plot_"*string(i+16)*".png")
end
=#
