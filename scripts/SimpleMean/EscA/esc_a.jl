# # Script de Evaluación de inflación media simple y promedios móviles 

# Escenario A: réplica del trabajo efectuado en 2020 (criterios básicos a dic-19) 
# utilizando la información hasta diciembre de 2019
...
savepath = datadir("results",medida,Esc)

# 3. Usamos run_batch, para gnenerar la evluación de la media simple
run_batch(gtdata, dict_sm, savepath,savetrajectories=true)

# ## Medias Móviles

avrange = 2:12
wmfs = [InflationMovingAverage(InflationSimpleMean(),i) for i in avrange]
...
# Usamos run_batch, para gnenerar la evluación de las medias móviles de la media simple interanual
...
using Chain
using PrettyTables
df_sm = collect_results(savepath)
...
plot!(InflationSimpleMean(), gtdata, fmt = :svg)
plot!(InflationMovingAverage(InflationSimpleMean(),3), gtdata, fmt = :svg)
