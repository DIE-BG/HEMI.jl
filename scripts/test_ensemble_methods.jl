...
totalfn = TotalCPI()
totalevalfn = CPIDataBase.TotalEvalCPI()

newdata = deepcopy(gtdata)

## Construct EnsembleFunction object
ensemblefn = EnsembleFunction(totalfn, TotalCPI())
ensemblefn(newdata)

# Construct ensemble functions that contains another ensemble function 
ensemblefn2 = EnsembleFunction(ensemblefn, totalfn)
ensemblefn2(newdata)
...
combfn2 = CombinationFunction(totalfn, TotalCPI(), totalfn, Float32[0.1, 0.4, 0.5])
combfn2(newdata)
