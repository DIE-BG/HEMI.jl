...
ls_metrics = get_metrics_2020(ls_results, testconfig, testdata; metrics_config...)
# Gráfica de trayectorias con ponderaciones ajustadas hasta diciembre de 2020
plot_trajectories_2020(ls_metrics, plots_path, "ls_combination_2020")
...
# Métricas de evaluación 
ridge_metrics = get_metrics_2020(ridge_results, testconfig, testdata; metrics_config...)
# Gráfica de trayectorias con ponderaciones ajustadas hasta diciembre de 2020
plot_trajectories_2020(ridge_metrics, plots_path, "ridge_combination_2020")
...
# Métricas de evaluación 
share_metrics = get_metrics_2020(share_results, testconfig, testdata; metrics_config...)
# Gráfica de trayectorias con ponderaciones ajustadas hasta diciembre de 2020
plot_trajectories_2020(share_metrics, plots_path, "share_combination_2020")
...
f(r) = !(r.scenario in ["A", "B", "C", "D"]) && !(r.method in ["ridge", "ls"])
f(r) = !(r.scenario in ["A", "C", "D"])
f(r) = true
...
## Gráficas de resultados modificables 

ridgefn20, comp_ridge20 = get_components_2020(compilation_results, "ridge", "E")
sharefn20, comp_share20 = get_components_2020(compilation_results, "share", "E")
lsfn20, comp_ls20 = get_components_2020(compilation_results, "ls", "E")

dates = Date(2001, 12):Year(1):Date(2020, 12)

# plotly()
gr()
plot(InflationTotalCPI(), gtdata, alpha = 0.7,
    xticks = (dates, Dates.format.(dates, dateformat"u-yy")),
    xrotation = 45
)
plot!(optmse2019, gtdata, linewidth = 2, color = :black)

plot!(lsfn20, gtdata, lw=1.5, color=:orange, alpha=0.8)
plot!(ridgefn20, gtdata, lw=2, color=:red, alpha=0.8)
plot!(sharefn20, gtdata, lw=3, color=:blue)

hline!([3], alpha=0.5, color = :gray, label=false)

savefig(joinpath(plots_path, "ls-ridge-share.svg"))