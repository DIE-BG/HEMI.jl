...
Esta función genera la trayectoria paramétrica , las trayectorias de simulación  y 
las métricas de evaluación utilizando la configuración [`SimConfig`]. 

Genera el error cuadrático medio `mse`, el error estándar de simulación `std_sim_error`, 
la raíz del error cuadrático medio `rmse`, el error medio `me` y el error absoluto medio `mae`, 
así como un array con las trayectorias de simulación `tray_infl`.
...
### Utilización
Esta función utiliza la función `evalsim` para generar un set de simulaciones en base a un `CountryStructure`
y un `AbstractConfig`, y genera un diccionario `results` con todas las métricas de evaluación y 
con la información del `AbstractConfig` utilizado para generarlas. Adicionalmente genera un objeto con las
trayectorias de inflación.

#### Ejemplos
`makesim` recibe un `CountryStructure` y un `AbstractConfig`, para trasladarlo a `evalsim` y generar las simulaciones.
Almacena las métricas y los parámetros de simulación en el diccionario results, y Adicionalmente devuelve las
trayectoria de simulacion.
...
La función run_batch genera paquetes de simulaciones con base en diccionario de parámetros de configuración.

### Utilización 
La función recibe un `CountryStructure`, un diccionario con vectores que contienen parámetros de simulación y
un directorio para almacenar archivos con las métricas de cada una de las evaluaciones generadas.

#### Ejemplo
Generamos un diccionario con parámetros de configuración para percentiles equiponderados, desde el 
percentil 60 hasta el percentil 80. Esto genera un diccionario con 21 configuraciones distintas para evaluación.
...
dict_prueba = Dict(
    ...
    :nsim => 1000) |> dict_list`
``` 
Una vez creado dict_prueba, podemos generar el paquete de simulación utilizando run_batch.
```julia-repl 
...
Esta función lee los resultados desde `savepath` y los presenta en un DataFrame.
